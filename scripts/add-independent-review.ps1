...
$mutexName='Global\Phase1-D48-'+([Convert]::ToHexString((Get-Sha256Bytes ([Text.Encoding]::UTF8.GetBytes([IO.Path]::GetFullPath($ReviewStorePath)))))).Substring(0,24)
...
try{
 ...
 $digest=[Convert]::ToHexString((Get-Sha256Bytes $bytes)).ToLowerInvariant();$id='{0:000000}-{1}'-f(@($index.generations).Count+1),$digest.Substring(0,16)
