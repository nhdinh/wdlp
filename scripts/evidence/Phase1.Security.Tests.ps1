...
$signed=Invoke-Verify $manifest -Signed;Assert ($signed.Exit-eq0) 'canonical manifest must pass signed-off validation';Assert ([string]::IsNullOrWhiteSpace($signed.Err)) 'expected empty stderr';$j=$signed.Out|ConvertFrom-Json;Assert ($j.status-eq'valid'-and@($j.diagnostics).Count-eq0) 'signed-off validation must have no diagnostics'
$cases=@(
