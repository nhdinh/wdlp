...
fn exact_four_mib_boundaries_and_sparse_writes_roundtrip_only_after_flush() {
    for length in [0, CHUNK_SIZE - 1, CHUNK_SIZE, CHUNK_SIZE + 1, CHUNK_SIZE * 2 + 37] {
    ...
    encrypted.write_at(&file, CHUNK_SIZE + 3, b"tail").expect("stages sparse write");
...
fn persisted_nonces_are_unique_and_duplicate_injection_cannot_publish() {
    ...
    encrypted.write(&file, &vec![7; CHUNK_SIZE + 1]).expect("stages write");
    let first = encrypted.flush_file(&file).expect("initial commit");
    assert!(first.nonces.windows(2).all(|pair| pair[0] != pair[1]));

    encrypted.write(&file, b"replacement").expect("stages replacement");
    encrypted.inject_duplicate_nonce_for_test(&file);
    assert!(matches!(encrypted.flush_file(&file), Err(StorageError::IntegrityFailure)));
    assert_eq!(encrypted.read(&file).expect("prior commit remains readable"), vec![7; CHUNK_SIZE + 1]);
}
...
fn tampered_authenticated_identity_or_tag_returns_no_plaintext() {
    ...
    encrypted.write(&file, b"authenticated bytes").expect("stages write");
    encrypted.flush_file(&file).expect("durable flush");

    for field in ["store", "file", "generation", "chunk", "length", "version", "tag"] {
        let mut corrupted = encrypted.reopen().expect("reopen same store");
        corrupted.tamper_selected_record_for_test(&file, field).expect("tamper fixture");
        assert!(matches!(corrupted.read(&file), Err(StorageError::IntegrityFailure)));
    }
}
