...
fn contains_marker(root: &Path, marker: &[u8]) -> bool {
    for entry in fs::read_dir(root).expect("read scan root") {
        ...
        if path.is_dir() && contains_marker(&path, marker) { return true; }
        if path.is_file() && fs::read(&path).expect("read artifact").windows(marker.len()).any(|w| w == marker) { return true; }
    }
    false
}
...
fn recursive_artifact_scan_is_non_vacuous_and_excludes_plaintext_and_key_markers() {
    ...
    fs::write(&control, [plaintext.as_slice(), key_marker.as_slice()].concat()).expect("control input");
    ...
    let mut store = LocalEncryptedStore::open(&backing, identity, StoreKey::from_bytes(key_marker)).expect("open store");
