# Research: Stack — Windows DLP Solution

## Domain

A centrally managed Windows Data Leakage Prevention solution built in Rust, with a user-mode virtual drive (WinFsp), encrypted local storage, and a Linux-based management server deployed via Docker Compose.
...
1. **WinFsp runtime dependency**: The `winfsp` crate only provides bindings; the WinFsp MSI must be installed on every endpoint. Bundle the installer in your deployment package and verify the driver version at agent startup.
2. **rustls crypto provider on Windows**: `rustls` 0.23 defaults to `aws-lc-rs`, which requires a C compiler and CMake. Pin the `ring` provider in `ClientConfig`/`ServerConfig` builds to avoid cross-compilation pain.
3. **windows-service SCM deadlines**: The service control manager will terminate a service that does not report `Running` within its deadline. Initialize Tokio and WinFsp quickly, report `StartPending` with a realistic `wait_hint`, and move long work (e.g., enrollment) onto a spawned task.
4. **SQLx compile-time checks need a DB or offline data**: Either run PostgreSQL during development/CI or commit `sqlx-data.json` with `sqlx prepare`. Forgetting this breaks builds in fresh environments.
5. **Nonce reuse in AEAD**: A single reuse of a (key, nonce) pair in `chacha20poly1305` or `aes-gcm` breaks confidentiality. Derive per-file keys or use a deterministic nonce scheme (e.g., file path + version counter + HKDF) and never randomize nonces without a counter.
6. **Policy bundle canonicalization**: When signing JSON bundles, sign a canonical byte representation and verify before deserialization. Do not sign `serde_json::to_string_pretty` output and then compare against a compact representation.
7. **Agent offline clock skew**: Offline policy expiration relies on the endpoint clock. Pair the seven-day server-signed expiry with tamper-evident local storage of the last bundle timestamp; a local clock rollback should not extend enforcement indefinitely.
8. **Windows path semantics in WinFsp**: Test NTFS-isms (case preservation, reserved names, reparse points, stream handling) early. Explorer and Office rely on subtle behaviors that differ from POSIX filesystems.
9. **Feature-gate bloat**: Both `windows` and `tokio` have many features. Enable only what you need in each crate to keep compile times and binary size reasonable.
10. **Secrets in logs**: `tracing` will happily serialize a `SecretString` if you use `?` or `%`. Either implement `Value`/`Display` carefully or redact fields before logging.
