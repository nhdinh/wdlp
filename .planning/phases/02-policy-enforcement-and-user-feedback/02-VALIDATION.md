...
> Finalized per-phase validation contract for feedback sampling during execution. The six-plan graph contains thirteen executable tasks; every task is mapped below, including its TDD/Wave 0 owner.
...
| 02-05-01 | 02-05 | 5 | DRV-05,08, UI-02 | T-02-05-01/02/05 | Authenticated exact Proceed-once grant is atomic, expiring, one-use, and fail-closed | integration tracer | `rtk cargo test --locked -p dlp-windows-service --test companion_grants warn_grant_tracer --quiet && rtk cargo test --locked -p dlp-windows-drive --test policy_enforcement warn --quiet` | task RED | pending |
...
- [ ] `crates/dlp-agent-core/tests/policy_activation.rs` — Plan 02-03 Tasks 1-2; signed activation, key rotation, restart, and LKG failure coverage.
...
- [x] All thirteen finalized tasks have an automated verification command and named Wave 0 owner.
