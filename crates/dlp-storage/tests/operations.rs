...
use dlp_storage::{
    CapturedStoreIdentity, LocalEncryptedStore, PathError, StorageError, StoreKey, VirtualPath,
};
...
fn store(sid: &str, store: &str) -> LocalEncryptedStore {
    LocalEncryptedStore::open(
        ...
        CapturedStoreIdentity::new(
            UserSid::parse(sid).expect("SID"),
            StoreId::parse(store).expect("store ID"),
        ),
...
fn paths_are_bounded_case_insensitive_and_cannot_escape_a_sid_store() {
    ...
    assert_eq!(
        VirtualPath::parse(r"documents/QUARTERLY REPORT.TXT")
            .expect("case fold")
            .lookup_key(),
        valid.lookup_key()
    );
...
fn file_directory_and_handle_operations_are_deterministic() {
    ...
    let handle = encrypted
        .create_or_open(&report, true, true)
        .expect("create file");
    ...
    assert_eq!(
        encrypted
            .read_path(
                &VirtualPath::parse(r"documents\\QUARTERLY report.TXT").expect("case lookup")
            )
            .expect("read"),
        b"dra"
    );
...
fn sids_handles_and_failed_publication_remain_isolated() {
    ...
    let second_handle = second
        ...
        .expect("second open");
    first
        .write_handle(first_handle, 0, b"first")
        .expect("first write");
    first.flush_handle(first_handle).expect("first flush");
    second
        .write_handle(second_handle, 0, b"second")
        .expect("second write");
    ...
    first
        .write_handle(replacement, 0, b"replacement")
        .expect("stage replacement");
    first.inject_write_failure_for_test();
    assert!(matches!(
        first.flush_handle(replacement),
        Err(StorageError::IoFailure | StorageError::NoSpace)
    ));
    assert_eq!(
        first
            .read_path(&path)
            .expect("prior generation stays readable"),
        b"first"
    );
}
